...
class Patient(BaseModel):
    ...
    # bmi is also a field that is computed based on other fields

    @computed_field     #@computed_field: A decorator used to define fields in a Pydantic model whose values are computed dynamically based on other fields in the model.
    @property           #@property: A built-in Python decorator that allows defining methods in a class that can be accessed like attributes, enabling dynamic computation of values.
    def bmi(self) -> float:
        height_in_meters = 1.75  # Assuming a fixed height for demonstration purposes
